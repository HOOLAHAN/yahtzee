...
const LeaderboardPlayerDrawer = ({ score, currentUserId, busy, message, onChallenge, onClose }: PlayerDrawerProps) => {
  const isCurrentUser = Boolean(currentUserId && score.userId === currentUserId);
  const canChallenge = Boolean(currentUserId && score.userId && !isCurrentUser);
  const sent = message.startsWith('Challenge sent');
  return <div className="leaderboard-player-overlay" onClick={onClose}>
    <section role="dialog" aria-modal="true" aria-labelledby="leaderboard-player-title" className="leaderboard-player-drawer" onClick={(event) => event.stopPropagation()}>
      <header className="leaderboard-player-header">
        <div><p className="eyebrow">{score.aggregate ? 'Weekly Daily Challenge' : score.mode === 'DAILY' ? 'Daily Challenge' : 'Solo game'}</p><h3 id="leaderboard-player-title">{score.username}</h3></div>
        <button type="button" onClick={onClose} aria-label="Close player details">&times;</button>
      </header>
      <div className="leaderboard-player-body">
        <strong className="leaderboard-player-score">{score.score}</strong>
        {(score.completedAt ?? score.timestamp) && <time>{new Date(score.completedAt ?? score.timestamp).toLocaleString()}</time>}
        {canChallenge ? <div className="leaderboard-challenge-action">
          <p><strong>Play {score.username}</strong><span>Send an invitation for a two-device Remote Game.</span></p>
          <button type="button" disabled={busy || sent} onClick={onChallenge}>{busy ? 'Sending…' : sent ? '✓ Challenge sent' : 'Challenge to Remote Game'}</button>
          {message && !sent && <small>{message}</small>}
        </div> : isCurrentUser ? <div className="leaderboard-self-label">This is your leaderboard entry</div> : <div className="leaderboard-self-label">Sign in to challenge this player</div>}
        {score.aggregate ? <p className="mt-4 text-sm text-mintGlow">Weekly score: the total of this player’s best five Daily Challenge results.</p> : score.completedAt ? <>
          <div className="mt-5 grid grid-cols-3 gap-2"><div className="rounded-xl bg-deepBlack p-3 text-center"><strong className="block text-2xl text-neonCyan">{score.yahtzeeCount ?? 0}</strong><small className="text-gray-500">Yahtzees</small></div><div className="rounded-xl bg-deepBlack p-3 text-center"><strong className="block text-2xl text-neonCyan">{score.earnedUpperBonus ? '✓' : '—'}</strong><small className="text-gray-500">Upper bonus</small></div><div className="rounded-xl bg-deepBlack p-3 text-center"><strong className="block text-2xl text-neonCyan">{score.noZeroScores ? '✓' : '—'}</strong><small className="text-gray-500">No zeroes</small></div></div>
          <ScorecardBreakdown value={score.scorecard} />
        </> : <p className="mt-4 text-sm leading-6 text-mintGlow">This is a historical leaderboard score. Detailed game statistics were not recorded for older entries.</p>}
      </div>
    </section>
  </div>;
};
...
const Leaderboard: React.FC<LeaderboardProps> = ({ showUserScores, hideHeading = false, onShowUserScoresChange, canShowUserScores = true }) => {
  ...
  const sendChallenge = async (score: LeaderboardEntry) => {
    ...
    setChallengeMessage('');
    try {
      await challengeLiveGame(score.userId, score.username);
    ...
    <div className={hideHeading ? '' : 'max-w-xl mx-auto p-6 bg-deepBlack rounded-2xl shadow-xl border-4 border-neonCyan'}>
      ...
      <section className="leaderboard-controls" aria-label="Leaderboard filters">
        <div className="leaderboard-selects">
          <label><span>GAME TYPE</span>{showUserScores ? <select value={historyMode} onChange={(event) => setHistoryMode(event.target.value as 'ALL' | ResultMode)}><option value="ALL">All games</option><option value="SOLO">Solo</option><option value="DAILY">Daily Challenge</option><option value="COMPUTER">Vs Computer</option><option value="PASS">Pass &amp; Play</option><option value="REAL">Real Dice</option><option value="REMOTE">Remote Game</option></select> : <select value={competition} onChange={(event) => { const next = event.target.value as Competition; setCompetition(next); setPeriod(next === 'daily' ? 'today' : period === 'today' ? 'week' : period); }}><option value="solo">Solo</option><option value="daily">Daily Challenge</option></select>}</label>
      ...
      {selected && <LeaderboardPlayerDrawer score={selected} currentUserId={userDetails?.userId} busy={challengeBusy} message={challengeMessage} onChallenge={() => void sendChallenge(selected)} onClose={() => setSelected(null)} />}
