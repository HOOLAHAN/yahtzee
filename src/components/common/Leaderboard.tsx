    ...
    <div className={hideHeading ? '' : 'max-w-xl mx-auto p-6 bg-deepBlack rounded-2xl shadow-xl border-4 border-neonCyan'}>
      ...
      <section className="leaderboard-controls" aria-label="Leaderboard filters">
        <div className="leaderboard-selects">
          <label><span>GAME TYPE</span>{showUserScores ? <select value={historyMode} onChange={(event) => setHistoryMode(event.target.value as 'ALL' | ResultMode)}><option value="ALL">All games</option><option value="SOLO">Solo</option><option value="DAILY">Daily Challenge</option><option value="COMPUTER">Vs Computer</option><option value="PASS">Pass &amp; Play</option><option value="REAL">Real Dice</option></select> : <select value={competition} onChange={(event) => { const next = event.target.value as Competition; setCompetition(next); if (next === 'solo' && period === 'today') setPeriod('week'); }}><option value="solo">Solo</option><option value="daily">Daily Challenge</option></select>}</label>
