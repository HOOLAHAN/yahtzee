...
const Navbar: React.FC<NavbarProps> = ({ activePage, pageTitle = 'Yahtzee!', onNavigate, playButtonLabel = 'Play', onPlayButtonClick, registrationRequest = 0 }) => {
  const [showAuthModal, setShowAuthModal] = useState(false);
  const [isMenuOpen, setIsMenuOpen] = useState(false);
  const [openDropdown, setOpenDropdown] = useState<'player' | 'account' | null>(null);
  ...
  return <>
    <nav ref={navRef} className="sticky top-0 z-30 border-b-2 border-neonCyan bg-deepBlack/95 px-4 py-3 text-white shadow-lg backdrop-blur"><div className="mx-auto flex max-w-7xl items-center justify-between gap-6">
      ...
      <div className="ml-auto hidden items-center gap-1 md:flex" aria-label="Primary navigation">
        <button onClick={() => { setOpenDropdown(null); onPlayButtonClick ? onPlayButtonClick() : go('play'); }} className={`desktop-nav-button ${activePage === 'play' ? 'desktop-nav-active' : ''}`}>{playButtonLabel}</button>
